...
const SignUpForm = () => {
  ...
  const onSubmit = async (data: z.infer<typeof signUpSchema>) => {
    ...
    setAuthError(null);
    try {
      // Sign up user to clerk
      await signUp.create({
        emailAddress: data.email,
        password: data.password,
      });
      // Sending email code to user via clerk
      ...
      setAuthError(error.error?.[0].message || "Error");
    } finally {
  ...
  const handleVerification = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Clerk will verify the OTP
      const result = await signUp.attemptEmailAddressVerification({
        code: verificationCode,
      });

      // If sign up successful then clerk will create new session
      ...
      setVerificationErr(e.error?.[0].message || "Error");
    } finally {
      setIsSubmitting(false);
    }
  };
